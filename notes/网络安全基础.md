...
- h=hash(m)
- one-way function，单向性，不能根据最终的哈希值 h，得到原始的数据 message
...
向 CA 机构申请证书的过程：
1. 向 CA 机构提供自己的信息：自己的机构名等信息（类似办理身份证），自己的公钥
2. CA 机构验证提交的信息后，加上颁发机构信息，证书的有效期，签名算法，
3. 
4. 如果颁发机构不是根证书颁发机构，
加上上级颁发机构证书，直到最上层的根证书颁发机构，加上证书的有效期，等信息
1. CA 颁发机构用自己的私钥对数据进行数字签名，

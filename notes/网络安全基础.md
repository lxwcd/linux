...
其中 shared secret value `K` 是服务端利用客户端的公钥和自己的生成的临时的私钥生成的（该过程通过 Diffie-Hellman 算法得到）, 共享密钥所用的算法为前面协商的算法  
...
1. **资源所有者（Resource Owner）**：拥有受保护资源的用户，例如一个社交媒体用户拥有自己的照片。  
2. **客户端（Client）**：请求访问资源的第三方应用程序，例如一个移动应用或网站。  
3. **授权服务器（Authorization Server）**：负责验证资源所有者的身份并颁发访问令牌（Access Token）的服务器，通常是提供认证服务的服务提供商，如Facebook或Google。  
4. **资源服务器（Resource Server）**：存储受保护的资源，例如照片或文档。  
...
2. **访问令牌（Access Token）**：访问令牌是客户端用来访问受保护资源的令牌，代表了用户授权的证明。  

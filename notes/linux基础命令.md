...
## setsid 脱离终端
1. **setsid 是底层标准方案**，用于创建独立会话，彻底脱离控制终端；
2. 常用于编写**守护进程 daemon**、长期后台服务；
3. 对比 `nohup &`：`nohup` 仅屏蔽挂断信号，未脱离会话/终端；`setsid` 从会话层面彻底断开终端，更彻底。

进程调用 `setsid()` 系统调用成功后：
1. 生成**全新会话 Session**，当前进程 = 会话首进程，`SID = 自身PID`；
2. 生成**全新进程组 Process Group**，当前进程 = 进程组组长，`PGID = 自身PID`；
3. **解绑原有控制终端 /dev/pts/x**，进程无控制终端（tty列显示 `?`）；
4. 不再归属原Shell会话，关闭终端/SSH窗口，进程不会收到 `SIGHUP` 信号，不会被杀。

### 限制
**进程如果是当前进程组组长，无法调用 setsid，会失败**。
Shell 命令 `setsid 程序` 能成功的原因：
Shell 先 `fork()` 生成子进程（子进程不是组长），再在子进程执行 `setsid`，规避限制。

### 和 nohup & 对比区分
| 方式                 | 是否新建会话     | 是否脱离终端         | jobs 能否查到 | 关闭终端行为           | 缺点                       |
| -------------------- | ---------------- | -------------------- | ------------- | ---------------------- | -------------------------- |
| `sleep 1000 &`       | 否，同bash会话   | 否，绑定终端         | 能查到        | 发SIGHUP，进程退出     | 一关终端就挂               |
| `nohup sleep 1000 &` | 否，同bash会话   | 逻辑脱离，会话仍绑定 | 能查到        | 屏蔽SIGHUP，可存活     | 仍归属原会话，终端IO会阻塞 |
| `setsid sleep 1000`  | 是，全新独立会话 | 彻底无控制终端       | 完全看不到    | 不受终端影响，稳定运行 | 无法用fg/jobs切回前台      |
...
4. 查看进程必须用全量ps：
```bash
ps -ef -o pid,ppid,pgid,sid,tty,comm | grep demo
```
输出特征：
`tty` 字段为 `?`，`SID` 等于程序自身PID。
...
### 配合日志输出
setsid 默认不会重定向输出，程序打印会丢失，搭配重定向：
```bash
setsid ./demo > app.log 2>&1
```
- `> app.log`：标准输出写入日志文件
- `2>&1`：错误输出同步写入日志

### 守护进程标准双fork原理
业务程序想要成为标准后台服务，固定两步fork：
1. 第一次fork，父进程退出，子进程不再是进程组组长；
2. 子进程调用 `setsid`，新建会话、脱离终端；
3. 二次fork，再次创建子进程执行业务，避免会话首进程重新抢占终端。

#### 适合用 setsid
1. 嵌入式设备长期后台服务（你的OK8MP开发板程序）；
2. 服务端常驻进程、守护程序；
3. 需要彻底和SSH/串口终端解绑，长期稳定运行；
4. 不想依赖Shell的jobs任务管理。

### 不适合用 setsid
1. 需要随时 `fg` 切回前台调试的临时任务；
2. 简单短期后台任务，临时跑几分钟，用 `nohup &` 更简单。

### 总结
1. 默认 `ps` 只打印当前终端进程，setsid 启动的程序必须加 `-e` 参数才能看见；
2. `jobs` 只管理当前Shell会话内任务，setsid 新建会话后不在列表；
3. `setsid` 程序父进程：Shell存在时PPID为bash PID，关闭Shell后PPID变为1（systemd）；
4. 仅 `nohup` 无法完全脱离会话，复杂后台服务优先 `setsid`。
...
**nohup**：后台任务未与终端解绑，但忽略 SIGHUP 信号，终端关闭或用户注销后任务仍会继续运行。用 jobs 看不到进程，用 ps 可以在另一个窗口查看进程。
...
## nohup 和 setsid
**nohup 不会和终端解绑；setsid 才是真正彻底解绑终端**
两者关闭终端程序都能跑，但底层两件完全不同的事：
1. nohup：进程**还绑定原终端、留在原会话**，只是屏蔽 SIGHUP 挂断信号；
2. setsid：进程**脱离终端、新建独立会话**，从根源和原终端/会话切割。

### nohup ./app &
- 进程的 tty 字段是 `/dev/pts/0`（终端设备），进程持有终端文件句柄；
- 没有断开和终端设备的连接，只是不怕关门信号；
- 隐患：程序如果读取键盘输入，会永久阻塞卡死；终端销毁后程序读写IO会异常崩溃。

### setsid ./app
- 进程 tty 显示 `?`，不绑定任何终端设备；
- 彻底丢弃终端句柄，不存在读取终端阻塞、IO崩溃问题；

### 会话归属（Linux 内核会话 SID）
#### nohup
SID 和当前 bash 的 PID 一致，属于同一个会话；
整个会话的信号、任务管理都会影响它，`jobs` 命令能查到这个后台任务。

#### setsid
自动创建全新会话，进程自己是会话首进程（SID=自身PID）；
脱离原 bash 会话，`jobs` 完全看不到，不受当前 Shell 管控。

### 信号逻辑
#### nohup
关闭终端时，内核**依然会给它发送 SIGHUP 挂断信号**；
nohup 只是提前设置进程忽略该信号，硬扛住不退出；
手动执行 `kill -HUP 进程PID`，程序依旧能收到挂断信号。

#### setsid
进程属于全新独立会话，原终端关闭只会给**原bash会话**发信号；
这套信号根本不会传递到 setsid 创建的新会话，进程全程收不到 SIGHUP。

### 能否切回前台交互
#### nohup
属于当前shell后台任务，可用 `fg` 切回前台；
但一旦关闭终端窗口，就算重连SSH，也无法找回这个进程交互。

#### setsid
完全脱离终端，没有交互界面，永远无法切回前台操作、看不到实时输出。

### 表格对比
| 特性               | nohup xxx &          | setsid xxx             |
| ------------------ | -------------------- | ---------------------- |
| 是否解绑终端设备   | 否，tty为终端路径    | 是，tty显示?           |
| 是否新建独立会话   | 否，共享bash的SID    | 是，SID等于进程自身PID |
| 关闭终端收到SIGHUP | 收到，自动忽略       | 根本收不到信号         |
| jobs命令查看       | 可以查到             | 看不到                 |
| 程序读取终端输入   | 阻塞卡死             | 无终端，不会阻塞       |
| 支持fg切前台       | 临时可行，关终端失效 | 完全不支持             |

### 示例
```bash
nohup sleep 600 &
ps -ef -o pid,sid,tty,comm | grep sleep
# 输出 tty=/dev/pts/0，sid和bash一致
```

```bash
setsid sleep 600
ps -ef -o pid,sid,tty,comm | grep sleep
# 输出 tty=?，sid等于sleep自身PID
```

1. nohup 静默后台：`nohup ./app > /dev/null 2>&1 &`
2. setsid 彻底后台常驻：`setsid ./app > app.log 2>&1`
...
进程打开任何文件、设备、管道、终端、socket 后，内核会给这个资源分配一个**非负整数编号**，这个数字就是**文件描述符 file descriptor，fd**。

它是**进程私有的数组下标**，每个进程内部都有一张「文件描述符表」：
下标（fd数字） → 内核文件表 → inode（磁盘文件/终端设备）

- 只在当前进程内有效，进程之间fd互不干扰；
- 从0开始从小到大分配，最小可用数字优先使用。

## 进程默认自带3个标准文件描述符
程序一启动，内核自动打开3个终端设备，固定占用 0、1、2：
| fd编号 | 名称            | 含义           | 默认绑定  |
| ------ | --------------- | -------------- | --------- |
| 0      | stdin 标准输入  | 读取输入       | 键盘/终端 |
| 1      | stdout 标准输出 | 正常打印输出   | 终端屏幕  |
| 2      | stderr 标准错误 | 报错、异常信息 | 终端屏幕  |


## 底层三层结构
1. 进程：文件描述符表（仅属于该进程，存数字）
   fd=0、1、2、3、4...
2. 内核全局：文件表（保存读写偏移、打开模式）
3. 文件系统：inode（磁盘文件/终端/管道）

多个fd可以指向同一个内核文件表（`2>&1` 就是让fd2和fd1指向同一套文件表）

## 重定向原理
`2>&1` 含义：
复制 fd1 的内核文件指针给 fd2，让 标准错误 和 标准输出 指向同一个输出目标。
1. `cmd > /dev/null` = `cmd 1>/dev/null`
修改fd1指向黑洞设备；
2. `cmd 2>&1`
让fd2复制fd1当前指向的设备。

`cmd 2>&1 >/dev/null`
1. 先执行`2>&1`：此时fd1指向屏幕，fd2跟着指向屏幕；
2. 再执行`1>/dev/null`：只修改fd1，fd2仍然指向屏幕，报错依旧打印。
...
exec 5> log.txt
```
分配fd=5，绑定文件log.txt，echo 内容可以写入：
```bash
echo test >&5
```
关闭自定义fd：`exec 5>&-`
...
### 管道 | 产生新fd
`ps | cat`
shell 创建管道，左边进程stdout(fd1)指向管道写端，右边进程stdin(fd0)指向管道读端。

### /dev/null 黑洞设备
是系统特殊设备文件，进程打开后分配fd，写入数据直接丢弃。

### 终端设备 /dev/pts/0
普通ssh终端的0、1、2全部绑定这个终端fd；
setsid 进程解绑终端，0/1/2无有效终端，读写不会阻塞卡死。
...
```
- `->` 后面就是fd绑定的资源
- setsid启动的进程：0/1/2 会指向 `/dev/null`，无终端
...
## 关键特性
1. **进程独立**
进程A的fd=3 和进程B的fd=3 完全无关，指向不同文件。
2. 进程退出，内核自动回收所有fd，释放资源。
3. 复制fd不会复制文件内容，仅复制指向内核资源的指针（轻量操作）。
4. nohup 底层逻辑：
程序标准输出fd1、错误fd2不再指向终端，自动重定向到nohup.out。
5. 为什么后台读取输入会阻塞：
fd0(stdin)绑定终端，后台无键盘输入，进程永久等待；setsid会把fd0指向空，不会阻塞。

## 文件描述符 vs inode 
1. 文件描述符 fd：**进程层面**的数字编号，进程私有的索引；
2. inode：**磁盘文件系统层面**的文件唯一标识，分区内全局；
一条链路：fd → 文件表 → inode。
多个fd可以指向同一个inode（软硬链接、重定向都会出现）。
